...
$(function(){
	...
	//右侧导航鼠标悬停提示
	var tipArray=new Array("旅游","名车","文化","古玩","TOP");
	var atex="";
	$("#right_nav a").hover(function(){
		var tex=$("#right_nav a").eq($("#right_nav a").index(this)).text();
		atex=tex;
		$("#right_nav a").eq($("#right_nav a").index(this)).css({
			'background':'#d10e03',
			'color':'#fff',
		});
		$("#right_nav a").eq($("#right_nav a").index(this)).text(tipArray[$("#right_nav a").index(this)]);
	},function(){
		$("#right_nav a").eq($("#right_nav a").index(this)).css({
			'background':'#eee',
			'color':'#666',
		});
		$("#right_nav a").eq($("#right_nav a").index(this)).text(atex);
	});
